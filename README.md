...
```
Run 20260505_041206: 5 processed / 4 skipped / 1 failed / 0 empty / 4 unmapped records - validation: PASSED
...
files in roughly inode-order, on NTFS it can be insertion-order, on
APFS the b-tree iteration order may vary by case-folding rules. Even
on a single machine, repeated runs after add/delete operations can
shift the order. Cross-OS, it's basically random.
...
`dirnames.sort()` sorts in place, which is what `os.walk` reads to
decide which subdirectories to descend into next, so the *traversal*
order is deterministic too. `sorted(filenames)` is purely about which
file inside a directory the loop emits next.

The effect is observable: try `python -m sanitizer ...` on the same
input from different OSes — the manifest rows come out in the same
order, the per-extension histogram is built in the same order, the
sanitized output tree is identical.
...
`-- tests/
    ...
    |-- test_pipeline.py    # 18 cases - dispatch, isolation, PII reports, idempotency, CSV format
...
You should see `43 passed` in roughly 1 second. The suite covers:
